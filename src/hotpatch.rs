...
impl std::fmt::Debug for Cache {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Cache").field("_path", &self.path).finish()
    }
}
...
pub fn create_jump_table(patch: &Path, triple: &Triple, cache: &Cache) -> Result<JumpTable> {
...
fn create_windows_jump_table(patch: &Path, cache: &Cache) -> Result<JumpTable> {
    use pdb::FallibleIterator;
...
fn create_native_jump_table(patch: &Path, triple: &Triple, cache: &Cache) -> Result<JumpTable> {
    let old_name_to_addr = &cache.symbol_table;
...
fn create_wasm_jump_table(patch: &Path, cache: &Cache) -> Result<JumpTable> {
